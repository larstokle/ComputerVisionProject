...
if  ~isempty(state)
    ...
    if use_AckermannConstraint
        ...
        if nnz(matches) < 20 % constraint may have been violated
            disp('Too few matches with Ackermann, trying without')
    ...
    if nnz(matches) < 20 % try with higher lambda to get more matches
        disp('Too few matches, trying with higher match lambda')
        matches = matchDescriptorsLocally(prev_descriptors, descriptors,...
                                          prev_corners, corners,...
                                          match_lambda*1.5, max_corner_dist);
    end
