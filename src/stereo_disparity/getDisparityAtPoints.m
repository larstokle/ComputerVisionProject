function disp_img = getDisparityAtPoints(...
    left_img, right_img, points, patch_radius, min_disp, max_disp)
...
for i = 1:size(points,2)
    ...
    row = p(1);
    col = p(2);
    if row < (1 + patch_radius) || row > (rows-patch_radius)...
            || col < (1 + max_disp + patch_radius) || col > (cols - patch_radius)
        continue
    end
